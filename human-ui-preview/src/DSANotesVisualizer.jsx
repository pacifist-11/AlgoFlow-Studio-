...
export const toFullExecutableProgram = (rawSnippet, lang, title = 'Algorithm') => {
  ...
  const hasDriverJS = /console\.log\s*\(/i.test(trimmed) && trimmed.length > 300;
  ...
  if (lang === 'C++') {
    ...
    if (codeBody.includes('kmpSearch') || codeBody.includes('buildLPS')) {
      mainCall += `    string txt = "ABABDABACDABABCABAB";\n    string pat = "ABABCABAB";\n    cout << "Text: " << txt << endl;\n    cout << "Pattern: " << pat << endl;\n    vector<int> matches = kmpSearch(txt, pat);\n    cout << "Matches found at indices: ";\n    for (int idx : matches) cout << idx << " ";\n    cout << endl;\n`;
    } else if (codeBody.includes('computeZ')) {
      mainCall += `    string s = "aabzaa";\n    vector<int> z = computeZ(s);\n    cout << "String: " << s << endl;\n    cout << "Z-Array: ";\n    for (int val : z) cout << val << " ";\n    cout << endl;\n`;
    } else if (codeBody.includes('rabinKarp')) {
      mainCall += `    string txt = "GEEKS FOR GEEKS";\n    string pat = "GEEK";\n    vector<int> res = rabinKarp(txt, pat);\n    cout << "Matches at: ";\n    for (int i : res) cout << i << " ";\n    cout << endl;\n`;
    } else if (codeBody.includes('minDistance')) {
      mainCall += `    string s1 = "intention", s2 = "execution";\n    cout << "Edit Distance between '" << s1 << "' and '" << s2 << "': " << minDistance(s1, s2) << endl;\n`;
    } else if (codeBody.includes('tspBitmask')) {
      mainCall += `    vector<vector<int>> dist = {{0, 20, 42, 25}, {20, 0, 30, 34}, {42, 30, 0, 10}, {25, 34, 10, 0}};\n    cout << "TSP Min Cost: " << tspBitmask(dist) << endl;\n`;
    } else if (codeBody.includes('dinicMaxFlow') || codeBody.includes('Dinic')) {
      mainCall += `    Dinic dinic(4);\n    dinic.addEdge(0, 1, 10);\n    dinic.addEdge(0, 2, 5);\n    dinic.addEdge(1, 2, 15);\n    dinic.addEdge(1, 3, 10);\n    dinic.addEdge(2, 3, 10);\n    cout << "Dinic Max Flow (0 -> 3): " << dinic.maxFlow(0, 3) << endl;\n`;
    } else if (codeBody.includes('millerRabin')) {
      mainCall += `    vector<long long> tests = {29, 31, 35, 97, 100};\n    for (long long v : tests) {\n        cout << "Is " << v << " prime? " << (millerRabin(v) ? "YES" : "NO") << endl;\n    }\n`;
    } else if (codeBody.includes('blellochScan')) {
      mainCall += `    vector<int> arr = {1, 2, 3, 4, 5, 6, 7, 8};\n    cout << "Blelloch Parallel Scan: ";\n    vector<int> res = blellochScan(arr);\n    for (int x : res) cout << x << " ";\n    cout << endl;\n`;
    } else if (codeBody.includes('infixToPostfix')) {
      mainCall += `    string exp = "(A+B)*C-D/E";\n    cout << "Infix: " << exp << endl;\n    cout << "Postfix: " << infixToPostfix(exp) << endl;\n`;
    } else if (codeBody.includes('sieve')) {
      mainCall += `    int n = 30;\n    cout << "Primes up to " << n << ": ";\n    vector<bool> primes = sieve(n);\n    for (int i = 2; i <= n; i++) if (primes[i]) cout << i << " ";\n    cout << endl;\n`;
    } else if (codeBody.includes('findMax')) {
    ...
    } else if (codeBody.includes('reverseArray') || codeBody.includes('reverseList')) {
      mainCall += `    cout << "Reverse execution completed successfully." << endl;\n`;
    } else {
  ...
  if (lang === 'Java') {
    ...
    if (trimmed.includes('kmpSearch') || trimmed.includes('KMPMatcher') || trimmed.includes('buildLPS')) {
      mainCall += `        String txt = "ABABDABACDABABCABAB";\n        String pat = "ABABCABAB";\n        System.out.println("Text: " + txt);\n        System.out.println("Pattern: " + pat);\n        int[] lps = KMPMatcher.buildLPS(pat.toCharArray());\n        System.out.println("LPS Table: " + Arrays.toString(lps));\n        System.out.println("Matches: " + Arrays.toString(KMPMatcher.kmpSearch(txt.toCharArray(), pat.toCharArray())));\n`;
    } else if (trimmed.includes('computeZ') || trimmed.includes('ZAlgorithm')) {
      mainCall += `        String s = "aabzaa";\n        System.out.println("Input: " + s);\n        System.out.println("Z-Array: " + Arrays.toString(ZAlgorithm.computeZ(s.toCharArray())));\n`;
    } else if (trimmed.includes('RabinKarp') || trimmed.includes('rabinKarp')) {
      mainCall += `        String txt = "GEEKS FOR GEEKS";\n        String pat = "GEEK";\n        System.out.println("Matches: " + Arrays.toString(RabinKarpEngine.search(txt.toCharArray(), pat.toCharArray())));\n`;
    } else if (trimmed.includes('EditDistanceEngine') || trimmed.includes('minDistance')) {
      mainCall += `        String s1 = "intention", s2 = "execution";\n        System.out.println("Edit Distance: " + EditDistanceEngine.minDistance(s1.toCharArray(), s2.toCharArray()));\n`;
    } else if (trimmed.includes('BitmaskTSPEngine') || trimmed.includes('solveTSP')) {
      mainCall += `        int[][] dist = {{0, 20, 42, 25}, {20, 0, 30, 34}, {42, 30, 0, 10}, {25, 34, 10, 0}};\n        System.out.println("TSP Min Cost: " + BitmaskTSPEngine.solveTSP(dist));\n`;
    } else if (trimmed.includes('DinicEngine') || trimmed.includes('dinic')) {
      mainCall += `        DinicEngine d = new DinicEngine(4);\n        d.addEdge(0, 1, 10);\n        d.addEdge(0, 2, 5);\n        d.addEdge(1, 2, 15);\n        d.addEdge(1, 3, 10);\n        d.addEdge(2, 3, 10);\n        System.out.println("Dinic Max Flow (0 -> 3): " + d.maxFlow(0, 3));\n`;
    } else if (trimmed.includes('VertexCover2Approx')) {
      mainCall += `        int[][] edges = {{0, 1}, {1, 2}, {2, 3}, {3, 0}};\n        System.out.println("2-Approx Cover: " + Arrays.toString(VertexCover2Approx.get2ApproxCover(4, edges)));\n`;
    } else if (trimmed.includes('MillerRabinEngine') || trimmed.includes('millerRabin')) {
      mainCall += `        long[] tests = {29, 31, 35, 97, 100};\n        for (long val : tests) {\n            System.out.println("Is " + val + " prime? " + MillerRabinEngine.millerRabin(val, 5));\n        }\n`;
    } else if (trimmed.includes('BlellochParallelEngine') || trimmed.includes('blellochScan')) {
      mainCall += `        int[] arr = {1, 2, 3, 4, 5, 6, 7, 8};\n        System.out.println("Parallel Scan: " + Arrays.toString(BlellochParallelEngine.blellochScan(arr)));\n`;
    } else if (trimmed.includes('InfixToPostfix') || trimmed.includes('infixToPostfix')) {
      mainCall += `        String exp = "(A+B)*C-D/E";\n        System.out.println("Infix: " + exp);\n        System.out.println("Postfix: " + InfixToPostfix.convert(exp));\n`;
    } else if (trimmed.includes('SieveOfEratosthenes') || trimmed.includes('sieve')) {
      mainCall += `        System.out.println("Primes up to 30: " + Arrays.toString(SieveOfEratosthenes.sieve(30)));\n`;
    } else if (trimmed.includes('findMax')) {
      mainCall += `        int[] arr = {12, 35, 1, 10, 34, 1};\n        System.out.println("Input Array: " + Arrays.toString(arr));\n        System.out.println("Maximum Value: " + new Main().findMax(arr));\n`;
    } else if (trimmed.includes('countEvens')) {
      mainCall += `        int[] arr = {3, 8, 12, 5, 9, 14};\n        System.out.println("Input Array: " + Arrays.toString(arr));\n        System.out.println("Count of Even Numbers: " + new Main().countEvens(arr));\n`;
    } else if (trimmed.includes('factorial')) {
    ...
    } else if (trimmed.includes('binarySearch')) {
    ...
    // Check if trimmed snippet contains any top-level class definition
    if (/^\s*(public\s+)?class\s+\w+/m.test(trimmed)) {
      // Demote "public class X" to "class X" so it can sit cleanly beside "public class Main"
      const sanitized = trimmed.replace(/public\s+class\s+(\w+)/g, 'class $1');
      return `${includes}${sanitized}\n\npublic class Main {\n${mainCall}}`;
    } else {
  ...
  if (lang === 'Python') {
    ...
    if (codeBody.includes('kmp_search') || codeBody.includes('build_lps')) {
      mainCall += `    txt = "ABABDABACDABABCABAB"\n    pat = "ABABCABAB"\n    print("Text:", txt)\n    print("Pattern:", pat)\n    print("Matches at indices:", kmp_search(txt, pat))\n`;
    } else if (codeBody.includes('compute_z')) {
      mainCall += `    s = "aabzaa"\n    print("String:", s)\n    print("Z-Array:", compute_z(s))\n`;
    } else if (codeBody.includes('rabin_karp')) {
      mainCall += `    txt = "GEEKS FOR GEEKS"\n    pat = "GEEK"\n    print("Matches:", rabin_karp(txt, pat))\n`;
    } else if (codeBody.includes('min_distance')) {
      mainCall += `    s1, s2 = "intention", "execution"\n    print(f"Edit Distance between '{s1}' and '{s2}':", min_distance(s1, s2))\n`;
    } else if (codeBody.includes('tsp_bitmask')) {
      mainCall += `    dist = [[0, 20, 42, 25], [20, 0, 30, 34], [42, 30, 0, 10], [25, 34, 10, 0]]\n    print("TSP Min Cost:", tsp_bitmask(dist))\n`;
    } else if (codeBody.includes('dinic_max_flow') || codeBody.includes('Dinic')) {
      mainCall += `    d = Dinic(4)\n    d.add_edge(0, 1, 10)\n    d.add_edge(0, 2, 5)\n    d.add_edge(1, 2, 15)\n    d.add_edge(1, 3, 10)\n    d.add_edge(2, 3, 10)\n    print("Dinic Max Flow (0 -> 3):", d.max_flow(0, 3))\n`;
    } else if (codeBody.includes('miller_rabin')) {
      mainCall += `    tests = [29, 31, 35, 97, 100]\n    for val in tests:\n        print(f"Is {val} prime? {miller_rabin(val)}")\n`;
    } else if (codeBody.includes('blelloch_scan')) {
      mainCall += `    arr = [1, 2, 3, 4, 5, 6, 7, 8]\n    print("Parallel Scan:", blelloch_scan(arr))\n`;
    } else if (codeBody.includes('infix_to_postfix')) {
      mainCall += `    exp = "(A+B)*C-D/E"\n    print("Infix:", exp)\n    print("Postfix:", infix_to_postfix(exp))\n`;
    } else if (codeBody.includes('sieve')) {
      mainCall += `    res = sieve(30)\n    primes = [i for i, p in enumerate(res) if p]\n    print("Primes up to 30:", primes)\n`;
    } else if (codeBody.includes('find_max') || codeBody.includes('findMax')) {
  ...
  if (lang === 'C') {
    ...
    if (codeBody.includes('kmpSearch') || codeBody.includes('buildLPS')) {
      mainCall += `    char txt[] = "ABABDABACDABABCABAB";\n    char pat[] = "ABABCABAB";\n    printf("Text: %s\\nPattern: %s\\n", txt, pat);\n    int matches[100];\n    int count = kmpSearch(txt, pat, matches);\n    printf("Matches found at indices: ");\n    for (int i = 0; i < count; i++) printf("%d ", matches[i]);\n    printf("\\n");\n`;
    } else if (codeBody.includes('computeZ')) {
      mainCall += `    char s[] = "aabzaa";\n    int z[20];\n    computeZ(s, z);\n    printf("String: %s\\nZ-Array: ", s);\n    for (int i = 0; i < (int)strlen(s); i++) printf("%d ", z[i]);\n    printf("\\n");\n`;
    } else if (codeBody.includes('rabinKarp')) {
      mainCall += `    char txt[] = "GEEKS FOR GEEKS";\n    char pat[] = "GEEK";\n    int matches[50];\n    int count = rabinKarp(txt, pat, matches);\n    printf("Matches at: ");\n    for (int i = 0; i < count; i++) printf("%d ", matches[i]);\n    printf("\\n");\n`;
    } else if (codeBody.includes('minDistance')) {
      mainCall += `    char s1[] = "intention", s2[] = "execution";\n    printf("Edit Distance: %d\\n", minDistance(s1, s2));\n`;
    } else if (codeBody.includes('dinicMaxFlow')) {
      mainCall += `    int cap[4][4] = { {0, 10, 5, 0}, {0, 0, 15, 10}, {0, 0, 0, 10}, {0, 0, 0, 0} };\n    printf("Dinic Max Flow (0 -> 3): %d\\n", dinicMaxFlow(4, 0, 3, cap));\n`;
    } else if (codeBody.includes('millerRabin')) {
      mainCall += `    long long tests[] = {29, 31, 35, 97, 100};\n    for (int i = 0; i < 5; i++) {\n        printf("Is %lld prime? %s\\n", tests[i], millerRabin(tests[i], 5) ? "YES" : "NO");\n    }\n`;
    } else if (codeBody.includes('infixToPostfix')) {
      mainCall += `    char exp[] = "(A+B)*C-D/E";\n    char out[100];\n    infixToPostfix(exp, out);\n    printf("Infix: %s\\nPostfix: %s\\n", exp, out);\n`;
    } else if (codeBody.includes('sieve')) {
      ...
      mainCall += `    int arr[] = {2, 7, 11, 15};\n    int outL, outR;\n    if (twoSum(arr, 4, 9, &outL, &outR)) {\n        printf("Two Sum (target 9) Indices: [%d, %d]\\n", outL, outR);\n    }\n`;
    } else if (codeBody.includes('binarySearch')) {
      mainCall += `    int arr[] = {1, 3, 5, 7, 9, 11, 13, 15};\n    printf("Binary search target 7 found at index: %d\\n", binarySearch(arr, 8, 7));\n`;
    } else {
  ...
  if (codeBody.includes('kmpSearch') || codeBody.includes('buildLPS')) {
    mainCall += `const txt = "ABABDABACDABABCABAB";\nconst pat = "ABABCABAB";\nconsole.log("Text:", txt);\nconsole.log("Pattern:", pat);\nconsole.log("Matches found at indices:", kmpSearch(txt, pat));\n`;
  } else if (codeBody.includes('computeZ')) {
    mainCall += `const s = "aabzaa";\nconsole.log("String:", s);\nconsole.log("Z-Array:", computeZ(s));\n`;
  } else if (codeBody.includes('rabinKarp')) {
    mainCall += `const txt = "GEEKS FOR GEEKS";\nconst pat = "GEEK";\nconsole.log("Matches at:", rabinKarp(txt, pat));\n`;
  } else if (codeBody.includes('minDistance')) {
    mainCall += `console.log("Edit Distance ('intention', 'execution'):", minDistance("intention", "execution"));\n`;
  } else if (codeBody.includes('tspBitmask')) {
    mainCall += `const dist = [[0, 20, 42, 25], [20, 0, 30, 34], [42, 30, 0, 10], [25, 34, 10, 0]];\nconsole.log("TSP Min Cost:", tspBitmask(dist));\n`;
  } else if (codeBody.includes('dinicMaxFlow')) {
    mainCall += `const edges = [[0, 1, 10], [0, 2, 5], [1, 2, 15], [1, 3, 10], [2, 3, 10]];\nconsole.log("Dinic Max Flow (0 -> 3):", dinicMaxFlow(4, 0, 3, edges));\n`;
  } else if (codeBody.includes('millerRabin')) {
    mainCall += `[29, 31, 35, 97, 100].forEach(n => console.log('Is ' + n + ' prime? ' + millerRabin(BigInt(n))));\n`;
  } else if (codeBody.includes('blellochScan')) {
    mainCall += `console.log("Blelloch Parallel Scan:", blellochScan([1, 2, 3, 4, 5, 6, 7, 8]));\n`;
  } else if (codeBody.includes('infixToPostfix')) {
    mainCall += `console.log("Postfix of (A+B)*C-D/E:", infixToPostfix("(A+B)*C-D/E"));\n`;
  } else if (codeBody.includes('sieve')) {
    mainCall += `const isPrime = sieve(30);\nconst primes = [];\nfor (let i = 2; i <= 30; i++) if (isPrime[i]) primes.push(i);\nconsole.log("Primes up to 30:", primes);\n`;
  } else if (codeBody.includes('findMax')) {
...
// Struct (User-defined Reference type)
struct User {
    char name[50];
    int age;
};
...
int main() {
    ...
    // Aggregate types
    struct User user1 = {"Alice", 25};
    int primes[] = {2, 3, 5, 7, 11};
    
    printf("Age: %d, Price: %.2f, Grade: %c, Active: %d\\n", age, price, grade, isActive);
    printf("User: %s (%d), Prime[0]: %d\\n", user1.name, user1.age, primes[0]);
    return 0;
}`
    },
  },
...
int main() {
    ...
    // Relational & Logical
    int isGreater = (a > b);
    int logicalAnd = (a > 5 && b < 5);
    
    // Bitwise
    int bitwiseXor = a ^ b;
    
    printf("Sum: %d, Rem: %d, Greater: %d, And: %d, XOR: %d\\n", sum, rem, isGreater, logicalAnd, bitwiseXor);
    return 0;
}`
    },
  },
...
// Pass by pointer (reference semantics)
...
int main() {
    ...
    printf("O(1) First: %d\\n", getFirst(nums, n));
    printf("O(N) Search 30: index %d\\n", linearSearch(nums, n, 30));
    return 0;
}`
    },
  },
...
// High-Performance Query Engine Architecture
int searchCorpus(const char* corpusText, const char* queryPattern, int matchIndices[]) {
    int n = strlen(corpusText);
    int m = strlen(queryPattern);
    int matchCount = 0;
    
    for (int i = 0; i <= n - m; i++) {
        int match = 1;
        for (int j = 0; j < m; j++) {
            if (corpusText[i + j] != queryPattern[j]) {
                match = 0;
                break;
            }
        }
...
// Algorithm Family Selector (Deterministic Routing)
const char* getAlgorithmFamily(const char* queryClass) {
    if (strcmp(queryClass, "PATTERN_SEARCH") == 0) return "String Algorithms (KMP / Z-Algo)";
    if (strcmp(queryClass, "FUZZY_MATCH") == 0)    return "Dynamic Programming (Wagner-Fischer)";
    if (strcmp(queryClass, "SIMILARITY") == 0)     return "Suffix Structures & Edit Distance";
    if (strcmp(queryClass, "ROUTING_FLOW") == 0)   return "Network Flow (Dinic Max Flow)";
    if (strcmp(queryClass, "HASH_INDEX") == 0)     return "Rabin-Karp Rolling Hash";
    return "Standard Linear Scan";
}`
    },
  },
...
// Zero-Dependency Dynamic Array Engine
typedef struct {
    int* data;
    int size;
    int capacity;
...
void initVector(CustomVector* vec) {
    vec->capacity = 4;
    vec->size = 0;
    vec->data = (int*)malloc(vec->capacity * sizeof(int));
}

void pushBack(CustomVector* vec, int value) {
    if (vec->size == vec->capacity) {
        vec->capacity *= 2;
        int* next = (int*)malloc(vec->capacity * sizeof(int));
        for (int i = 0; i < vec->size; i++) next[i] = vec->data[i];
        free(vec->data);
        vec->data = next;
    }
...
int kmpSearch(const char* text, const char* pattern, int* matches) {
    int n = strlen(text);
...
public class ZAlgorithm {
    public static int[] computeZ(char[] s) {
      ...
      C: `// Z-Algorithm in O(N) (C)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

void computeZ(const char* s, int* z) {
    int n = strlen(s);
    z[0] = 0;
    int l = 0, r = 0;
    for (int i = 1; i < n; i++) {
...
public class RabinKarpMatcher {
    public static int[] search(char[] text, char[] pat) {
      ...
      C: `// Rabin-Karp Rolling Hash (C)
#include <stdio.h>
#include <string.h>

int rabinKarp(const char* text, const char* pattern, int* matches) {
    const int d = 256, q = 101;
    int n = strlen(text), m = strlen(pattern);
    int p = 0, t = 0, h = 1, matchCount = 0;
...
public class EditDistanceEngine {
    public static int minDistance(char[] s1, char[] s2) {
      ...
      C: `// Wagner-Fischer Edit Distance DP (C)
#include <stdio.h>
#include <string.h>
...
int minDistance(const char* s1, const char* s2) {
...
public class BitmaskTSPEngine {
    public static int solveTSP(int[][] dist) {
      ...
      C: `// TSP Bitmask DP in O(2^N * N^2) (C)
#include <stdio.h>
#include <string.h>
...
int solveTSP(int mask, int u, int n, int dist[16][16]) {
    if (mask == (1 << n) - 1) return dist[u][0];
    if (memo[mask][u] != -1) return memo[mask][u];
    ...
    for (int v = 0; v < n; v++) {
        if (!(mask & (1 << v))) {
            int newCost = dist[u][v] + solveTSP(mask | (1 << v), v, n, dist);
...
int tspBitmask(int n, int dist[16][16]) {
    memset(memo, -1, sizeof(memo));
    return solveTSP(1, 0, n, dist);
}`
    },
  },
...
function dinicMaxFlow(n, source, sink, edges) {
  ...
  const to = [], cap = [], flow = [], next = [];
  let edgeCount = 0;

  function addEdge(u, v, c) {
    to[edgeCount] = v; cap[edgeCount] = c; flow[edgeCount] = 0; next[edgeCount] = head[u]; head[u] = edgeCount++;
  ...
  function dfs(u, pushed) {
    ...
    for (let cid = ptr[u]; cid !== -1; cid = next[cid]) {
      ptr[u] = cid;
      const v = to[cid];
      const tr = cap[cid] - flow[cid];
      if (level[u] + 1 === level[v] && tr > 0) {
        const push = dfs(v, Math.min(pushed, tr));
        if (push > 0) {
          flow[cid] += push;
          flow[cid ^ 1] -= push;
          return push;
  ...
  let maxFlow = 0;
  while (bfs()) {
    for (let i = 0; i < n; i++) ptr[i] = head[i];
    while (true) {
...
def dinic_max_flow(n, s, t, edges):
    head = [-1] * n
    to = []
    cap = []
    nxt = []
    ...
    def dfs(u, pushed):
        ...
        while e != -1:
            ptr[u] = e
            v = to[e]
            if level[u] + 1 == level[v] and cap[e] > 0:
                push = dfs(v, min(pushed, cap[e]))
                if push > 0:
                    cap[e] -= push
                    cap[e ^ 1] += push
                    return push
...
public class DinicEngine {
    ...
    private int n, s, t;
    private java.util.List<Edge>[] adj;
    private int[] level, ptr;

    @SuppressWarnings("unchecked")
    public DinicEngine(int n, int s, int t) {
    ...
    public void addEdge(int from, int to, int cap) {
        adj[from].add(new Edge(to, cap, 0, adj[to].size()));
    ...
    public int dfs(int v, int pushed) {
        ...
        for (int cid = ptr[v]; cid < adj[v].size(); ++cid) {
            ...
            int push = dfs(edge.to, Math.min(pushed, edge.cap - edge.flow));
            if (push == 0) continue;
            edge.flow += push;
            adj[edge.to].get(edge.rev).flow -= push;
            return push;
        }
        return 0;
    }
    ...
    public int maxFlow() {
        ...
        while (bfs()) {
            ...
            int pushed;
            while ((pushed = dfs(s, Integer.MAX_VALUE)) != 0) flow += pushed;
        }
        return flow;
    }
}`,
      C: `// Dinic Algorithm Max Flow (C)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
AdjList adj[100];
int level[100], ptr[100];
int q[100];

void addEdge(int from, int to, int cap) {
    if (adj[from].size == adj[from].capacity) {
        adj[from].capacity = adj[from].capacity ? adj[from].capacity * 2 : 4;
        adj[from].edges = (Edge*)realloc(adj[from].edges, sizeof(Edge) * adj[from].capacity);
    }
    if (adj[to].size == adj[to].capacity) {
        adj[to].capacity = adj[to].capacity ? adj[to].capacity * 2 : 4;
        adj[to].edges = (Edge*)realloc(adj[to].edges, sizeof(Edge) * adj[to].capacity);
    }
    adj[from].edges[adj[from].size++] = (Edge){to, cap, 0, adj[to].size};
    adj[to].edges[adj[to].size++] = (Edge){from, 0, 0, adj[from].size - 1};
}
...
int dfs(int v, int t, int pushed) {
    ...
    for (int cid = ptr[v]; cid < adj[v].size; ++cid) {
        ...
        int push = dfs(e->to, t, pushed < rem ? pushed : rem);
        if (push == 0) continue;
        e->flow += push;
        adj[e->to].edges[e->rev].flow -= push;
        return push;
    }
    return 0;
}

int dinicMaxFlow(int n, int s, int t) {
    int flow = 0;
    while (bfs(n, s, t)) {
...
public class VertexCover2Approx {
    public static int[] get2ApproxCover(int n, int[][] edges) {
      ...
      C: `// Vertex Cover 2-Approximation via Maximal Matching (C)
#include <stdio.h>
#include <stdbool.h>

int vertexCover2Approx(int numNodes, int edges[][2], int edgeCount, int* cover) {
    bool visitedEdge[edgeCount];
    bool inCover[numNodes];
    for (int i = 0; i < edgeCount; i++) visitedEdge[i] = false;
    for (int i = 0; i < numNodes; i++) inCover[i] = false;
...
bool millerRabin(long long n, int k = 5) {
    ...
    return true;
      ...
      Java: `// Miller-Rabin Engine
public class MillerRabinEngine {
    public static long power(long x, long y, long p) {
        long res = 1;
        x = x % p;
        while (y > 0) {
            if ((y & 1) == 1) res = (long)((java.math.BigInteger.valueOf(res).multiply(java.math.BigInteger.valueOf(x)).mod(java.math.BigInteger.valueOf(p))).longValue());
            y = y >> 1;
            x = (long)((java.math.BigInteger.valueOf(x).multiply(java.math.BigInteger.valueOf(x)).mod(java.math.BigInteger.valueOf(p))).longValue());
        }
        return res;
    }
    ...
    public static boolean millerRabin(long n, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            while (tempD != n - 1) {
                x = (long)((java.math.BigInteger.valueOf(x).multiply(java.math.BigInteger.valueOf(x)).mod(java.math.BigInteger.valueOf(n))).longValue());
                tempD *= 2;
                if (x == n - 1) { composite = false; break; }
            }
            if (composite) return false;
...
const PROBLEMS = {
  FIND_MAX: {
    ...
    notes: {
      ...
      C: `// Blelloch Parallel Prefix Scan (Up-Sweep & Down-Sweep) (C)
#include <stdio.h>
#include <math.h>

void blellochScan(int* a, int n) {
    // Up-sweep (reduce)
...
void inorderHelper(struct TreeNode* root, int* res, int* size) {
    if (!root) return;
    inorderHelper(root->left, res, size);
    res[(*size)++] = root->val;
    inorderHelper(root->right, res, size);
}
...
string infixToPostfix(string s) {
    ...
    for (char c : s) {
        if (isalnum(c)) out += c;
            ...
            while (!st.empty() && st.top() != '(' && getPrec(st.top()) >= getPrec(c)) {
                out += st.top(); st.pop();
            }
            st.push(c);
        }
    ...
    return out;
      ...
      Java: `public class InfixConverter {
...
void infixToPostfix(const char* exp, char* out) {
    char stack[256];
    int top = -1, outIdx = 0;
    for (int i = 0; exp[i] != '\0'; i++) {
        ...
        if (isalnum(ch)) {
            ...
            while (top >= 0 && stack[top] != '(') {
                out[outIdx++] = stack[top--];
            }
            if (top >= 0) top--; // pop '('
        } else {
            while (top >= 0 && stack[top] != '(' && prec(stack[top]) >= prec(ch)) {
                out[outIdx++] = stack[top--];
            }
            stack[++top] = ch;
        }
    }
    while (top >= 0) out[outIdx++] = stack[top--];
    out[outIdx] = '\0';
}`
    },
    generator: () => {
...
vector<bool> sieve(int n = 30) {
    ...
    return isPrime;
      ...
      Java: `public class SieveOfEratosthenes {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-primary)', position: 'relative', overflow: 'hidden' }}>
      ...
      {activeTab === 'notes' && (() => {
        ...
        if (isLesson) {
          ...
          const rawCode = lesson && lesson.code ? getSafeCode(lesson.code, activeLang) : '';
              ...
              <div style={{ background: 'var(--bg-secondary)', borderRadius: '16px', border: '1px solid var(--glass-border)', overflow: 'hidden' }}>
                ...
                <pre style={{
                  ...
                  {toAllman(activeCodeToDisplay).split('\n').map((lineText, idx) => (
                    <div key={idx} style={{ display: 'flex', padding: '1px 0', fontFamily: "'Fira Code', 'Cascadia Code', monospace" }}>
                      <span style={{ width: '38px', userSelect: 'none', textAlign: 'right', paddingRight: '14px', color: '#475569', fontSize: '0.85em', flexShrink: 0 }}>
                        {idx + 1}
                      </span>
                      <span style={{ whiteSpace: 'pre', color: '#f8fafc' }}>
                        {lineText || ' '}
                      </span>
                    </div>
                  ))}
      ...
      <CodeRunnerModal
        ...
        code={(() => {
          if (activeTab === 'notes') {
            const currentItem = LESSONS[selectedProbKey] || PROBLEMS[selectedProbKey] || LESSONS['LESSON_VARIABLES'];
            const safeC = getSafeCode(currentItem?.code, activeLang);
            return toFullExecutableProgram(safeC, activeLang, currentItem?.title);
          }
          const activeProb = PROBLEMS[selectedProbKey] || PROBLEMS['FIND_MAX'];
          return toFullExecutableProgram(rawActiveCode, activeLang, activeProb.title);
        })()}
        language={activeLang}
      />
    </div>
