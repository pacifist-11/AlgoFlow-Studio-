  ...
  return (
    <>
      ...
      <div style={{ display: !appMode ? 'block' : 'none', position: 'relative' }}>
        {/* Human-crafted Top Navigation Bar */}
        <header className="top-nav-bar">
          <div className="top-nav-inner">
            <div className="brand-badge" onClick={() => { setAppMode(null); setHomeSearchQuery(''); setCategoryFilter('ALL'); }}>
              <div className="brand-logo-icon">🌿</div>
              <div>
                <span className="brand-title">AlgoFlow Studio</span>
              </div>
              <span className="brand-version-tag">v2.5</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
              {isInstallable && (
                <button 
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0.45rem 0.95rem', borderRadius: '8px', background: 'rgba(16, 185, 129, 0.12)', border: '1px solid rgba(16, 185, 129, 0.3)', color: '#34d399', cursor: 'pointer', fontSize: '0.82rem', fontWeight: '600' }}
                >
                  📥 Install
                </button>
              )}
              ...
              <button 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0.45rem 0.95rem', borderRadius: '8px', background: 'rgba(129, 140, 248, 0.08)', border: '1px solid rgba(129, 140, 248, 0.25)', color: 'var(--accent-secondary)', cursor: 'pointer', fontSize: '0.82rem', fontWeight: '600' }}
              >
                ✨ Roadmap
              </button>
              <button 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0.45rem 0.95rem', borderRadius: '8px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--glass-border)', cursor: 'pointer', fontSize: '0.82rem' }}
        ...
        <div className="home-container">
          {!appMode && lastActiveMode && (
            <div className="resume-banner" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <h4 style={{ margin: 0, color: 'var(--text-primary)', fontSize: '0.9rem', fontWeight: 650 }}>Recent Workspace Session</h4>
                  <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.78rem' }}>
                    Continue where you left off in <strong>{
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn btn-start" style={{ padding: '0.35rem 0.85rem', fontSize: '0.8rem', borderRadius: '6px', margin: 0, width: 'auto' }} onClick={() => {
                  setAppMode(lastActiveMode);
                  setSetupComplete(lastSetupComplete);
                  window.location.hash = lastActiveMode;
                }}>Resume</button>
                <button className="btn btn-clear" style={{ padding: '0.35rem 0.65rem', fontSize: '0.8rem', borderRadius: '6px', margin: 0 }} onClick={() => {
          ...
          <div style={{ textAlign: 'center', marginBottom: '2.5rem', width: '100%' }}>
            ...
            <div style={{ 
              ...
              <span>AlgoFlow Engine v2.5</span>
              <span style={{ opacity: 0.35 }}>•</span>
              <span style={{ color: '#cbd5e1' }}>{welcomeRole ? `Workspace: ${welcomeRole}` : 'Open Lab'}</span>
            </div>

            <h1 className="title-gradient" style={{ fontSize: '2.85rem', marginBottom: '0.6rem', fontWeight: 800 }}>AlgoFlow Studio</h1>
            <p style={{ margin: '0 auto 2.2rem auto', maxWidth: '620px', color: '#94a3b8', fontSize: '1.02rem', lineHeight: 1.6 }}>
              Interactive data structures, visual execution algorithms, and step-by-step memory tracing crafted for engineers and learners.
            </p>
            ...
            <div style={{ 
              ...
              borderRadius: '10px',
              background: '#111319',
              border: `1.5px solid ${isSearchFocused ? 'rgba(56, 189, 248, 0.55)' : 'rgba(255, 255, 255, 0.09)'}`,
              boxShadow: isSearchFocused ? '0 0 0 3px rgba(56, 189, 248, 0.15)' : '0 2px 10px rgba(0, 0, 0, 0.25)',
              transition: 'all 0.2s ease',
              display: 'flex',
              alignItems: 'center',
              padding: '0.2rem 1rem'
            }}>
              <span style={{ fontSize: '0.95rem', marginRight: '8px', opacity: 0.45 }}>🔍</span>
              <input 
                type="text" 
                ...
                placeholder="Search modules (e.g. AVL Tree, Dijkstra, DP, Arrays)..."
                style={{ 
                  flex: 1, 
                  padding: '0.65rem 0', 
          ...
          {/* Category Filter Pills (When not in deep search) */}
          {homeSearchQuery.trim() === '' && (
            <div className="filter-pills-row">
              {[
                { id: 'ALL', label: 'All Modules', icon: '⚡' },
                { id: 'TREES', label: 'Trees & Heaps', icon: '🌲' },
                { id: 'LINEAR', label: 'Linear DSA', icon: '📦' },
                { id: 'SORT_SEARCH', label: 'Sorting & Search', icon: '📊' },
                { id: 'GRAPH_DP', label: 'Graphs & DP', icon: '🕸️' },
                { id: 'LEARNING', label: 'Learning & Debugger', icon: '📚' }
              ].map(pill => (
                <button
                  key={pill.id}
                  className={`filter-pill-btn ${categoryFilter === pill.id ? 'active' : ''}`}
          ...
          <div className="home-grid">
            {homeSearchQuery.trim() === '' ? (
              [
                { id: 'BEGINNER_101_VIS', category: 'LEARNING', icon: '🌱', tag: 'FOUNDATION', title: 'Beginner 101', desc: 'B.Tech Branch Roadmaps, Language Picker, Variables, Arrays & Sorting fundamentals.' },
                { id: 'GENERAL_DSA_VIS', category: 'LINEAR', icon: '📦', tag: 'LINEAR DSA', title: 'General DSA Visualizer', desc: 'Step through foundational linear structures: Stacks, Queues, Linked Lists, and Hash Tables.' },
                { id: 'SORT_SEARCH_VIS', category: 'SORT_SEARCH', icon: '📊', tag: 'ALGORITHMS', title: 'Sort & Search Visualizer', desc: 'Real-time array sorting and searching comparisons animated with dynamic time steps.' },
                { id: 'MAIN_VIS', category: 'TREES', icon: '🌲', tag: 'HIERARCHICAL', title: 'Tree Visualizer Studio', desc: 'Build BST, AVL, Min/Max Heaps, B-Tree, and Segment Trees step-by-step with code generator.' },
                { id: 'GRAPH_VIS', category: 'GRAPH_DP', icon: '🕸️', tag: 'GRAPH THEORY', title: 'Graph Visualizer Studio', desc: 'Construct customized weighted graphs. Animate BFS, DFS, Dijkstra, and Greedy best-first traversals.' },
                { id: 'DP_GREEDY_VIS', category: 'GRAPH_DP', icon: '🧠', tag: 'OPTIMIZATION', title: 'DP & Greedy Visualizer', desc: 'Visualize LCS, LIS, Knapsack, and Coin Change DP vs. Greedy side-by-side matrices.' },
                { id: 'JAVA_OOP_VIS', category: 'LEARNING', icon: '⚡', tag: 'PATTERNS', title: 'Patterns Visualizer Studio', desc: 'Animate and compile 10 different loop patterns (Pyramid, Pascal, Floyd, Butterfly) in 5 languages.' },
                { id: 'DSA_NOTES_VIS', category: 'LEARNING', icon: '📚', tag: 'THEORY & CODE', title: 'DSA Study & Solved Sums', desc: 'Comprehensive theory, 6 advanced algorithm modules, and interactive LeetCode/CodeChef problems.' },
                { id: 'LINE_BY_LINE_VIS', category: 'LEARNING', icon: '🐞', tag: 'EXECUTION TRACER', title: 'Line-by-Line Debugger', desc: 'Interactive step execution tracing. Inspect memory state, call frames, and variable states.' },
                { id: 'CODE_VAL_VIS', category: 'LEARNING', icon: '💻', tag: 'WORKSPACE', title: 'Code Validator & Runner', desc: 'Write or paste code in 5 languages. Enhanced syntax validation, error detection, and native cloud execution.' }
              ]
              ...
              [
                ...
                <div key={card.id} className="option-card" onClick={() => setPendingModule(card)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                    <div className="option-icon">{card.icon}</div>
                    <span style={{ 
                      fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', 
      ...
      {/* Global Footer with Human-Crafted Aesthetic */}
      <footer style={{ 
        display: 'flex', 
        justifyContent: 'center', 
        width: '100%', 
        ...
        <div style={{ 
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
              <span style={{ fontWeight: 700, color: '#f1f5f9', letterSpacing: '-0.01em' }}>AlgoFlow Studio</span>
              <span style={{ opacity: 0.3 }}>•</span>
              <span style={{ fontSize: '0.78rem', color: '#64748b' }}>Crafted for Engineers & CS Students</span>
            </div>
            <div style={{ color: '#64748b', fontSize: '0.78rem' }}>
              © {new Date().getFullYear()} Algorithm Visualizer Studio. All algorithms simulated client-side.
            </div>
          </div>
