...
async function dispatchRealEmail({ to, pin, name }) {
  ...
  const subject = `${pin} is your AlgoFlow verification PIN`;
  const htmlContent = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; background: #0f172a; color: #f8fafc; padding: 24px; border-radius: 12px; border: 1px solid #1e293b;">
      <h2 style="color: #38bdf8; margin: 0 0 12px 0;">⚡ AlgoFlow Studio</h2>
      <p style="color: #cbd5e1; font-size: 14px; margin-bottom: 20px;">Hello <strong>${recipientName}</strong>,</p>
      <p style="color: #cbd5e1; font-size: 14px; margin-bottom: 20px;">Use this 4-digit PIN to sign in to your AlgoFlow account:</p>
      <div style="background: rgba(56, 189, 248, 0.1); border: 1px solid #38bdf8; border-radius: 8px; padding: 16px; text-align: center; margin-bottom: 20px;">
        <span style="font-size: 34px; font-weight: 800; letter-spacing: 8px; color: #38bdf8;">${pin}</span>
      </div>
      <p style="color: #94a3b8; font-size: 12px; margin: 0;">This PIN will expire in 5 minutes. If you did not request this, you can ignore this email.</p>
    </div>
  `;
